// 망고아이 AI 상담직원 — Cloudflare Worker (정적 자산 + API)
// - 정적 파일(public/)은 자동으로 먼저 서빙됨. 이 Worker 는 /api/chat 같은 비-자산 경로만 처리.
// - Gemini 키는 환경변수(Secret) GEMINI_API_KEY 에서만 읽음 → 브라우저로 노출되지 않아 안전.
...
export default {
  async fetch(request, env) {
    ...
    // 그 외 비-자산 경로: 정적 자산으로 폴백(바인딩이 있을 때)
    if (env.ASSETS) {
